...
# Creates a network using a cppn and a substrate.
def create_phenotype_network(cppn, substrate, connection_maps, activation_function="relu"):
    ...
    del substrate[(1,0)]
    del substrate[(0,0)]
    hidden_coordinates = [(substrate[k], k) for k in substrate.keys()] # List of layers, first index = top layer.
    ...
    if len(hidden_nodes) != 0:
        ...
        for oc in output_coordinates[0]:
            ...
            for cm in connection_mappings:
                source_sheet = cm[0]
                im += find_neurons(cppn, oc, output_coordinates[1], (substrate[source_sheet], source_sheet), hidden_nodes[idx], False)  
        ...
        for i in range ((len(layers)-1), 2, -1):
            ...
            for j in range(len(layers[i])):
                ...
                for hc in substrate[target_sheet]:
                    ...
                    for cm in connection_mappings:
                        source_sheet = cm[0]
                        im += find_neurons(cppn, hc, target_sheet, (substrate[source_sheet], source_sheet), hidden_nodes[idx], False)       
                        idx += len(substrate[source_sheet])
                    if im:
        ...
        for i in range(len(layers[2])):
            ...
            # For each coordinate in target sheet
            for hc in substrate[target_sheet]:
                im = find_neurons(cppn, hc, target_sheet, (input_coordinates[0], input_coordinates[1]), input_nodes[idx], False)
                if im:
                    node_evals.append((hidden_nodes[counter], activation, sum, 0.0, 1.0, im))
                counter += 1
        ...
        for i in range(len(layers[0])):
            ...
            # For each coordinate in target sheet
            for oc in output_coordinates[0]:
                im = find_neurons(cppn, oc, output_coordinates[1], (input_coordinates[0], input_coordinates[1]), input_nodes[idx], False)
                if im:
                    node_evals.append((output_nodes[counter], linear_activation, sum, 0.0, 1.0, im))
                counter += 1
...
def query_cppn(coord1, coord2, outgoing, cppn, cppnon_tuple, max_weight=5.0):
    ...
    if abs(w) > 0.01:  # If abs(weight) is below threshold, treat weight as 0.0.
        return w #* max_weight
    else:
        return 0.0