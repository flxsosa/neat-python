...
def create_phenotype_network(cppn, substrate, activation_function="sigmoid"):
    ...
    for oc in output_coordinates:
        idx = 0
        for layer in hidden_coordinates:
            im = find_neurons(cppn, oc, layer[0], hidden_nodes[idx], False)
            idx += len(layer[0])
            if im:
    ...
    current_layer = 1
    idx = 0
    # Traverse the layers in the substrate
    for layer in hidden_coordinates:
        # print layer
        idx += len(layer[0])
        counter = idx - len(layer[0])
        for i in range(current_layer, len(hidden_coordinates)):
            for hc in layer[0]:
                im = find_neurons(cppn, hc, hidden_coordinates[i][0], hidden_nodes[idx], False)
                if im:
